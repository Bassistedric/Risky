...
def get_active_heepo_factor(
    ...
    if factor.family != family:
        raise HTTPException(
            status_code=400,
            detail=(
                f"Le facteur {factor.code} appartient Ã  la famille "
                f"{factor.family} et non Ã  {family}"
            ),
        )
...
# ============================================================
# CATALOGUE DES FACTEURS HEEPO
# ============================================================

@router.get(
    "/heepo-factors",
    response_model=list[schemas.HeepoFactorResponse],
)
...
def update_event_heepo(
    ...
    try:
        ...
        # ---------------------------------------------------------
        # 1. Validation complÃ¨te avant toute modification
        # ---------------------------------------------------------
            ...
            for item in family_items:
                ...
                if not is_other and item.other_text:
                    raise HTTPException(
                        status_code=400,
                        detail=(
                            f"Le texte libre est uniquement autorisÃ© "
        ...
        # ---------------------------------------------------------
        # 4. Snapshot APRÃˆS modification
        # ---------------------------------------------------------
        ...
        write_audit_log(
            ...
            details=f"Analyse HEEPO de l'Ã©vÃ©nement {event.event_number}",
        )
