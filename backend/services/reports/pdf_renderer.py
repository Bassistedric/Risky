...
I18N = {
    "fr": {
        ...
        "tree": "Arbre des causes", "actions": "Mesures & actions", "organization": "Organisation", "entity": "Entité", "trade": "Métier",
    ...
    "nl": {
        ...
        "tree": "Oorzakenboom", "actions": "Maatregelen & acties", "organization": "Organisatie", "entity": "Entiteit", "trade": "Vakgebied",
    ...
    "en": {
        ...
        "tree": "Cause tree", "actions": "Measures & actions", "organization": "Organization", "entity": "Entity", "trade": "Trade",
    ...
    "pl": {
        ...
        "tree": "Drzewo przyczyn", "actions": "Środki i działania", "organization": "Organizacja", "entity": "Jednostka", "trade": "Branża",
...
def render_accident_report_pdf(data: dict, db, language: str = "fr") -> bytes:
    ...
    if hasattr(raw_date, "strftime"):
        cover_date = raw_date.strftime("%d/%m/%Y")
    else:
        raw_date_text = _s(raw_date)
        try:
            from datetime import datetime
            cover_date = datetime.fromisoformat(raw_date_text.replace("Z", "+00:00")).strftime("%d/%m/%Y")
        except (ValueError, TypeError):
            cover_date = raw_date_text

    event_type_labels = {
        "fr": {"ACCIDENT": "Accident", "INCIDENT": "Incident", "NEAR_MISS": "Presqu’accident"},
        "nl": {"ACCIDENT": "Ongeval", "INCIDENT": "Incident", "NEAR_MISS": "Bijna-ongeval"},
        "en": {"ACCIDENT": "Accident", "INCIDENT": "Incident", "NEAR_MISS": "Near miss"},
        "pl": {"ACCIDENT": "Wypadek", "INCIDENT": "Incydent", "NEAR_MISS": "Zdarzenie potencjalnie wypadkowe"},
    }
    analysis_labels = {
        "fr": {"ADVANCED": "Circonstancié", "NORMAL": "Normal"},
        "nl": {"ADVANCED": "Omstandig", "NORMAL": "Normaal"},
        "en": {"ADVANCED": "Circumstantial", "NORMAL": "Normal"},
        "pl": {"ADVANCED": "Szczegółowa", "NORMAL": "Normalna"},
    }
    cover_rows = [
        [tr["entity"], _s(event.get("entity_name") or event.get("organization_name"))],
        [tr["trade"], _s(event.get("trade_name"))],
        [tr["event_type"], event_type_labels.get(lang, event_type_labels["fr"]).get(_s(event.get("event_type")), _s(event.get("event_type")))],
        [tr["analysis"], analysis_labels.get(lang, analysis_labels["fr"]).get(_s(event.get("analysis_type")), _s(event.get("analysis_type")))],
        [tr["event_date"], cover_date],
